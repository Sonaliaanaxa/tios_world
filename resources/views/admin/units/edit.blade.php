      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-body custom-edit-service">
            ...
            <!-- Add Blog -->
            <form method='post' action="{{route('units.update',$id)}}" enctype="multipart/form-data">
              @csrf
                ...
                <div class="row">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Unit Name') }}" value="{{$category->name}}" aria-required="true" />
                      @if ($errors->has('name'))
                    ...
                    <label>{{ __('Status*') }}</label>
									<div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
										<select class="custom-select {{ $errors->has('status') ? ' is-invalid' : '' }}" name='status' id="input-status">
											<option value='{{ $category->status }}'>Select Status</option>
