			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-body custom-edit-service">
						...
						<!-- Add Blog -->
						<form method='post' action="{{ route('trial-subcategories.create') }}" enctype="multipart/form-data">
							@csrf
							...
							<div class="service-fields mb-3">
								...
								<div class="col-sm-6 col-md-6">
										<label class="category">{{ __('Select Trial Category *')  }}</label>
									...
									<div class="form-group col-md-6">
										<label for="category">Trial Subcategory Name</label>
										<div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
											<input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Trial Subcategory Name') }}" value="{{ old('name') }}" aria-required="true" />
											@if ($errors->has('name'))
									...
									<div class="form-group col-md-6">
										<label for="category">Trial Subcategory Slug</label>
										<div class="form-group{{ $errors->has('slug') ? ' has-danger' : '' }}">
											<input class="form-control{{ $errors->has('slug') ? ' is-invalid' : '' }}" name="slug" id="input-slug" type="text" placeholder="{{ __('Trial Subcategory Slug') }}" value="{{ old('slug') }}" aria-required="true" />
											@if ($errors->has('slug'))
