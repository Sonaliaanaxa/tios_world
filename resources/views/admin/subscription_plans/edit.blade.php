					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card">
								...
								<!-- Add Blog -->
                <form method='post'  action="{{ route('subscription.plans.update',$id) }}"  enctype="multipart/form-data">
          @csrf
            @include('admin.layouts.flash_msg')
                  ...
                  <label class="col-sm-2 col-form-label">{{ __('Select Business Partners*')  }}</label>
                      <div class="col-sm-6 col-md-4">
                        ...
                        <select  class="custom-select {{ $errors->has('buspart_id') ? ' is-invalid' : '' }}business_partners" name='buspart_id' id="input-buspart_id"   >
                                ...
                                @foreach($business_partners as $c)
                                <option value='{{ $c->id}}' {{ ($c->id==$subscriptionplans->buspart_id)?'selected':''}} > {{ $c->name}} </option> 
                                @endforeach </select>
                ...
                <label class="col-sm-2 col-form-label">{{ __('Plan Name*') }}</label>
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('plan_name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('plan_name') ? ' is-invalid' : '' }}" name="plan_name" id="input-plan_name" type="text" placeholder="{{ __(' Plan Name') }}"  value="{{ $subscriptionplans->plan_name }}"  aria-required="true"/>
                ...
                <div class="row"> 
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('price') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}" name="price" id="input-price" type="number" placeholder="{{ __(' Price') }}"  value="{{ $subscriptionplans->price }}"  aria-required="true"/>
                      @if ($errors->has('price'))
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('tax') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('tax') ? ' is-invalid' : '' }}" name="tax" id="input-tax" type="number" placeholder="{{ __(' Tax %') }}"  value="{{ $subscriptionplans->tax }}"  aria-required="true"/>
                      @if ($errors->has('tax'))
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('tax_price') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('tax_price') ? ' is-invalid' : '' }}" readonly name="tax_price" id="input-tax_price" type="number" placeholder="{{ __(' Tax Price') }}" style='background-color:#fafafa;'  value="{{ $subscriptionplans->tax_price }}"  aria-required="true"/>
                  ...
                  <label class="col-sm-2 col-form-label">{{ __(' Currency*') }}</label>
                      <div class="col-sm-4">
                        ...
                        <select  class="custom-select {{ $errors->has('currency') ? ' is-invalid' : '' }}" name='currency' id="input-currency"   >
                                <option value='{{ $subscriptionplans->currency }}'>{{ $subscriptionplans->currency }}</option>
                                <option value='&#36' {{ ('dollars'==$subscriptionplans->currency)?'selected':''}}> US Dollars </option>  
                                <option value='&#8377' {{ ('rupee'==$subscriptionplans->currency)?'selected':''}}>  INR Rupee </option> 
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('total_price') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('total_price') ? ' is-invalid' : '' }}" readonly name="total_price" id="input-total_price" type="number" placeholder="{{ __('Total Price') }}"  value="{{ $subscriptionplans->total_price }}"  aria-required="true"/>
                ...
                <div class='row'>
                  ...
                  <div class="col-sm-4">
                    ...
                    <select  class="custom-select {{ $errors->has('status') ? ' is-invalid' : '' }}" name='status' id="input-status"   >
                            <option value='{{ $subscriptionplans->status }}'>{{ $subscriptionplans->status }}</option>
                            <option value='1' {{ ('1'==$subscriptionplans->status)?'selected':''}}> Activate </option>  
                            <option value='0' {{ ('0'==$subscriptionplans->status)?'selected':''}}> Deactivate</option>
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('vaild') ? ' has-danger' : '' }}">
                           <select  class="custom-select {{ $errors->has('vaild') ? ' is-invalid' : '' }}" name="vaild" id="input-vaild" >
                                <option value='{{ $subscriptionplans->vaild }}'>{{ $subscriptionplans->vaild }} Month</option>
                                <option value='1' {{ ('1'==$subscriptionplans->vaild)?'selected':''}}> 1 Month </option> 
                                <option value='3' {{ ('3'==$subscriptionplans->vaild)?'selected':''}}> 3 Month </option>  
                                <option value='6' {{ ('6'==$subscriptionplans->vaild)?'selected':''}}> 6 Month </option> 
                                <option value='12' {{ ('12'==$subscriptionplans->vaild)?'selected':''}}> 12 Month </option> 
                          </select>
                ...
                <div class="row"  id="file-content" >
                          ...
                          <br>
                          <img src="{{ asset('public/uploads/subscriptionplans') }}/{{ $subscriptionplans->img }}" style='margin-bottom:30px;height:200px;width:250px;border-radius:5%;'/>
                                <br>
