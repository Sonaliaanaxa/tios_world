					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card">
								...
								<!-- Add Blog -->
                <form method='post'  action="{{ route('trial-categories.create') }}"  enctype="multipart/form-data">
          @csrf
            @include('admin.layouts.flash_msg')
									<div class="service-fields mb-3">
										...
										<div class="form-group col-md-6">
                                                <label for="category">Trial Category Name</label>
                                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Trial Category Name') }}"  value="{{ old('name') }}"  aria-required="true"/>
                                                    @if ($errors->has('name'))
                                                        <span id="name-error" class="error text-danger" for="input-name">Trial Category Name is Empty!</span>
                                                    @endif
                                                </div>
                                            </div>
