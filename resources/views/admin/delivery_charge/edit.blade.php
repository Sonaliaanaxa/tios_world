					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card">
								...
								<!-- Add Blog -->
                <form method='post'  action="{{ route('delivery-charges.update',$id) }}"  enctype="multipart/form-data">
          @csrf
            @include('admin.layouts.flash_msg')
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('delivery_charges') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('delivery_charges') ? ' is-invalid' : '' }}" name="delivery_charges" id="input-delivery_charges" type="text" placeholder="{{ __(' Delivery Charges') }}"  value="{{ $product->delivery_charges }}"  aria-required="true"/>
                      @if ($errors->has('delivery_charges'))
                        <span id="delivery_charges-error" class="error text-danger" for="input-delivery_charges">Delivery Charge is Empty!</span>
                      @endif
                    </div>
                  </div>
