...
class AllbusMiddleware
{
 // /**
    ...
    public function handle($request, Closure $next)
    {
        ...
        if(auth()->user()->user_type == "doctor"){$valid=1;}
        if(auth()->user()->user_type == "hospital"){$valid=1;}
        if(auth()->user()->user_type == "diagnostic"){$valid=1;}
            if(auth()->user()->user_type == "blood_bank"){$valid=1;}
        if(auth()->user()->user_type == "pharmacy"){$valid=1;}
