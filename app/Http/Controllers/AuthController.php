...
class AuthController extends Controller
{
    ...
    public function userRegister(Request $request){
        ...
        if($usr){
            ...
            try{
                Mail::send(['html'=>'front/mailtemplate/user_registration'], $data, function($message)  use ($email,$name) {
                    $message->to($email,$name)->subject('Registration Notification');
                    $message->from('contact@hygieneherbs.in','The Hygiene Herbs Admin');
                });
            }catch(\Exception $e){
        ...
        $authentication = Auth::login($usr);
        if($authentication == true){
            Session::flash('success','You are registered and login successfuly!');
            return redirect()->back();
        }else{
            Session::flash('loginfalse','You are not authorized to login here!');
            return redirect()->back();
